...
use cranelift_wasm::{
	DefinedFuncIndex, FuncEnvironment, FuncIndex, FuncTranslator, Global, GlobalIndex, GlobalVariable, Memory,
...
impl JitosEnvironment {
	...
	/// Return the number of imported functions within this `JitosEnvironment`.
...
impl<'dummy_environment> JitosFuncEnvironment<'dummy_environment> {
	...
	fn vmctx_sig(&self, sigidx: SignatureIndex) -> ir::Signature {
		...
		sig.params.push(ir::AbiParam::special(self.pointer_type(), ir::ArgumentPurpose::VMContext));
		sig
	}
}
...
impl<'dummy_environment> FuncEnvironment for JitosFuncEnvironment<'dummy_environment> {
	...
	fn make_global(&mut self, func: &mut ir::Function, index: GlobalIndex) -> WasmResult<GlobalVariable> {
		...
		Ok(GlobalVariable::Memory { gv: vmctx, offset, ty: self.mod_info.globals[index].entity.ty })
	}
	...
	fn make_direct_func(&mut self, func: &mut ir::Function, index: FuncIndex) -> WasmResult<ir::FuncRef> {
		...
		Ok(func.import_function(ir::ExtFuncData { name, signature, colocated: false }))
	}
	...
	fn translate_call_indirect(
		...
		let vmctx = pos.func.special_param(ir::ArgumentPurpose::VMContext).expect("Missing vmctx parameter");
		...
		Ok(pos.ins().CallIndirect(ir::Opcode::CallIndirect, INVALID, sig_ref, args).0)
	}
	...
	fn translate_ref_func(&mut self, mut pos: FuncCursor, _func_index: u32) -> WasmResult<ir::Value> {
		Ok(pos.ins().null(self.reference_type()))
	}
